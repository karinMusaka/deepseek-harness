...
模型设置与产品引导插件。同一个 client Cordis 插件会注册 Models 页面和两个有序的首次使用弹窗：版本化内测声明，以及按条件显示的 DeepSeek 官方凭据步骤。两个步骤共用同一套弹窗组件，并继续由 `settings.onboarding` 排序。Models 平面把四个协议领域汇聚为一个共享快照：`llm.providers`（可配置提供方目录，含每条路由的存活／休眠状态）、`settings.describe`（序列化 schema、分层脱敏值、secret slot）、`credentials.describe`（不含值的 configured/source/writable 徽标），以及 `llm.models`（下文「按模型选择器可见性」分区所列的、未经过滤的 Host 模型 catalog）；页面据此渲染提供方行，一次只展开一张编辑卡片，且不把路由存活状态呈现为提供方状态。
...
提供方行与「新增」区块下方还有一个分区，标题为「选择器中显示的模型」，它列出未经过滤的 Host catalog（`llm.models`）中的每一个提供方分组——与任何会话自己的建议目录无关，因此本页隐藏过的模型仍会留在这里以便重新显示，而某个会话自己的目录因其他原因（一条已选中但不再公布的路由）当前隐藏的模型不受本分区影响。每个模型是一个复选框（勾选＝显示）；每个提供方还有一个 **全部显示** / **全部隐藏** 控件，可一次性切换整组。取消勾选会针对 `ui-model-selection.hiddenModels.<provider>` 写入 `unset`/`set` 这两种 `settings.mutate` 路径 op——某提供方的隐藏列表清空为空时用 `unset`，否则用带完整下一数组的 `set`——并携带该 namespace 当前的 revision；`settings-conflict` 响应会重新加载整个页面快照并显示重试提示，普通成功也同样重新加载，因此这个分区始终依据 Host 已接受的值渲染，而非乐观的本地猜测。namespace 与字段名是镜像的字符串常量，与本包 `apiKey.ts` 为其 `normalizeApiKey` 孪生体记录的做法相同：持有该 settings namespace 的插件是另一个包 `@deepseek-ai/dsh-client-ui-model-selection`，而客户端插件包之间禁止跨包值引入——本页读取该 namespace 已加载的视图，做法与它读取自己并不持有的 `llm-deepseek`/`llm-pi-ai` 完全一致。真正对选择器执行该偏好的唯一位置是 `ModelDirectory`（那个包的浏览器侧）；本分区只编辑偏好本身，从不触及任何会话的选择。catalog 拉取和上方的凭据补充信息一样是软加载：这里的传输或业务失败只会收窄本分区（以内联消息呈现），绝不会让提供方行编辑失败。当 settings 文档只读，或 `ui-model-selection` namespace 尚未加载（还没有可写入的地址）时，复选框会被禁用。写入进行期间做出的切换会排在它之后：每个排队的写入都基于上一次写入返回的 namespace 视图执行，因此携带该 revision，并作用于该隐藏列表；某次写入失败时，排在其后的切换会被丢弃，队列清空后页面重新加载一次。
...
- **可见性分区对每个用户展示同一份未过滤 catalog**：它自身没有按用户或按会话的过滤，只编辑一份文档级共享的 `hiddenModels` 偏好——如需按用户或进一步限定范围，那是 `ui-model-selection` 自己的限制。
- **在这里隐藏并不会撤销某个会话已经做出的选择**：一个当前选择正是某个已隐藏模型的会话仍会保留它（由 `ModelDirectory` 的豁免规则决定）；本分区既看不到也改不了任何会话的实时选择。
