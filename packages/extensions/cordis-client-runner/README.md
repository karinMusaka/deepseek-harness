...
- `loadStatic(request)` / `unloadStatic(pluginId)` — load or unload a browser half no host-runner definition owns: [`@deepseek-ai/dsh-cordis-static-packages-client`](../cordis-static-packages-client/README.md) supplies the identity, label, source, and its own `invoke`/`reportGuardFailure`/`reportRenderFailure` outlets (`DynamicCordisLoadOverrides`) in place of the environment's host-runner wiring, so a static package's routing and failure reports never reach the host runner's own Agent-scoped paths. Evaluation, guarding, module seating, and teardown ride the same engine as a host-defined load; `unloadStatic` is the awaitable counterpart to the `retract` event a host-runner definition receives, since a static package has no host-side stop to broadcast one.
