...
| [`cordis-static-packages/`](cordis-static-packages/README.md) | Boot-time loader for dual-half packages kept on disk: same sandbox and guard as a defined package, configured rather than approved per run | provides `ctx.staticCordisPackages` |
| [`cordis-static-packages-client/`](cordis-static-packages-client/README.md) | Browser half of static packages: fetches each configured source at page load and seats it through the dynamic-package runner | client face; loads through `ctx.dynamicCordisRunner` |
