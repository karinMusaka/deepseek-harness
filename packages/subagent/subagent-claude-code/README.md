...
Each query sets `persistSession: false` unless the call opted into resume (`request.requestResume`/`request.resumeId` — see "Resume (opt-in)" below). It supplies no elicitation or dialog callback, so an unattended interaction outside the fixed allowlist fails through a `canUseTool` denial instead of waiting for a user interface this provider does not own.

### Resume (opt-in)

This provider advertises the `resume` start-time capability. A caller that sets `request.requestResume: true` sets `persistSession: true` instead of the default `false`; a successful `completed` result then carries `resumeId: message.session_id`, the SDK's own session identity for that query. A later call that sets `request.resumeId` to a value the caller already holds passes it as the SDK's `Options.resume` to continue that exact session instead of starting fresh; `persistSession: true` is implied whenever `resumeId` is set, so the continued session itself stays resumable. `settingSources: []` and the fixed `canUseTool` permission scope remain enforced on a resumed call exactly like a fresh one — resuming grants no additional trust. `forkSession` is deliberately not used: this mechanism continues the SAME `session_id` across calls, it does not branch a new one from an existing history. Resuming from a cwd different from the one the session was created in, or resuming an id the SDK's own store does not recognize, is not a thrown exception — it is a normal `is_error: true` result message with an actionable `errors` array (e.g. `"No conversation found with session ID: …"`), so it reaches this provider's EXISTING failure-classification path (see "Failure classification" above) with no new classification code required. This capability is deployment-gated one layer up, in [`dsh-tool-subagent`](../tool-subagent/README.md#resume-opt-in)'s `allowResume` config key — a model can request resume only when the deployment has opted in.

## Capabilities and context

The provider advertises the `permissionMode` and `resume` start-time capabilities (enforced as above) and no other optional capability; it reports `inheritsParentContext: false`. Claude Code receives the standalone text task, the parent Session cwd, and the fixed permission scope, but not the parent conversation, persona, tool filter, depth policy, or structured-output contract. Every run has an independent SDK query, cancellation controller, and CLI process; the product session is non-persisted unless the call opted into resume.
...
- **One fresh query and process per run; session continuation is opt-in only** — there is no pooling or progress stream, and resume (see "Resume (opt-in)" above) keeps only the SDK's own session state, never this process — a resumed call still spawns a fresh `claude` process that reattaches to the persisted session by id.
- **A persistent (resumed) session is stored in the host's own `~/.claude/projects/` store, keyed by cwd** — unlike `persistSession: false`, which the SDK never writes past process exit, `persistSession: true` writes the session into the same on-disk, cwd-keyed store the user's own interactive `claude` sessions use, so a delegated session becomes visible in the user's own `claude --resume` picker. A deployment that opts into `allowResume` should treat this as a durable, host-visible side effect, not an internal implementation detail. Resuming from a cwd different from the one the session was created in fails (see "Resume (opt-in)" above) precisely because the store is cwd-keyed.
- **Replaying the parent's Session log does not reproduce a resumed child's internal state** — the parent's own log still faithfully reconstructs every model-visible tool call and result (the model-visible ⟺ logged invariant holds), but the CHILD's own SDK session evolves independently on each resume; replaying the parent's log a second time does not re-derive what the child's persisted session now contains, unlike a purely in-harness (non-resumable) child whose entire behavior is a deterministic function of the logged request.
