...
- `resume`：让子 agent 的提供方原生线程／会话在本次运行结束后继续存活，以便*之后*的某次调用续接它（`request.requestResume`／`request.resumeId`、`SubagentResult.resumeId`）。进程内提供方共享父级自身的 Cordis 权限，没有独立的原生线程需要持久化，因此 `spawn`／`fork`／`acp`／`dsh-sdk` 均声明为 `false`；`codex` 与 `claude-code` 是仅有的两个具备提供方原生可续接线程／会话概念的提供方。与 `permissionMode` 不同，这*确实*是一个每次调用都可以正当提出的请求，其门控放在再上一层的部署自身可选项中（`dsh-tool-subagent` 的 `allowResume` 配置项），而不是像 `permissionMode` 那样仅限部署配置——因为续接某次特定的既有运行（而非放宽范围）正是发起调用的模型本就有资格做出的那类决定。见下文「resume 授权」了解模型提供的 `resumeId` 在触达任何提供方之前是如何被验证的。
...
模型提供的 `request.resumeId` 是不可信输入——这个字符串是模型自己选的，不是 harness 选的——因此 `SubagentRuntime.start()` 会在任何提供方看到它*之前*先验证它，依据的是发起委派的 agent 自身的会话日志，绝不是进程内缓存：harness 会话可以在进程重启后存活，因此进程内的登记表会在重启后悄悄地不再为合法续接授权，却完全没有带来任何真正的安全性收益（攻击者只需等下一次重启即可绕过）。`verifyResumeIssuance(session, candidate)`（`src/resume-issuance.ts`）扫描 `session.events`，查找某个 `tool/result` 事件，其 `meta[SUBAGENT_RESUME_META_KEY]` 记录的 `{ id, provider, permissionMode, cwd }` 签发信息在*全部四个*字段上都与 `candidate` 匹配——而不仅仅是 `id`。按 `permissionMode` 与 `cwd` 做范围绑定（比较的是它们*有效的*、已完成默认值处理的值，绝不是原始的部署配置值）正是为了阻止这样一种情形：模型通过某个工具行（比如一个 `read-only` 的 `codex` 实例）启动了某个线程，之后却试图通过针对同一提供方、但范围更宽（比如 `workspace-write`）的另一个工具行续接同一个 id。一次续接被拒绝——无论是这个 id 从未被签发过，还是在不同范围下被签发的——都会产生*同一个* `RESUME_REJECTED` 错误：这是刻意为之，一个能够区分这两种失败的模型将白白获得一个探测预言机。`dsh-tool-subagent` 是唯一内置的写入方，负责写下这条签发记录（见其自身的 README）；本包只负责验证。
...
`SubagentRun.result` 兑现为 `{ output, structured?, stopReason, failure?, authMode?, changedFiles?, usage? }`。子 agent 级失败会以非 `completed` 原因兑现；只有 seam 无法表示的基础设施故障才可以拒绝。`dispose()` 是幂等的，会取消剩余工作，并等待结果结算以及子 agent 资源完全停稳。result 的拒绝只通过 `result` 本身报告；只有独立的资源释放失败，才会使 `dispose()` 被拒绝。`output` 与 `subagent/end` 事件的 `lastAssistantMessage` 使用导出的 `AssistantOutputFold`／`finalAssistantOutput` 辅助函数选取子 agent 最后一条非空 assistant 消息；若没有这类消息，则选取其累积的 assistant 文本。子 agent 两种输出均未产生时，`output` 为 `[]`，该事件字段缺省（结果约定归 [`SubagentResult.output`](../../../docs/subsystems/subagent.md#the-terminal-result-subagentresult) 所有）。

`failure` 与 `authMode` 只由进程外产品提供方（[`dsh-subagent-codex`](../subagent-codex/README.md)、[`dsh-subagent-claude-code`](../subagent-claude-code/README.md)）填充；任何进程内提供方都会让两者保持缺省。`failure` 把一个 `stopReason: 'error'` 的结果分类为封闭、不可合并扩展的 `SubagentFailureCode`（`'auth' | 'quota' | 'provider' | 'protocol'`）外加提供方自身的可操作文本，与 `stopReason` 本身正交——stop reason 回答"运行为什么结束"，`failure` 回答"这是哪一种失败"，并且只有当第一个问题的答案是 `'error'` 时才有意义。`authMode` 报告 `'api-key'`／`'subscription'`，纯粹从该提供方部署的 `Config.env` 是否设置了凭据形状的变量推导得出，绝不读取凭据存储文件。`dsh-tool-subagent` 复用本包自身的 `SubagentError`，把已分类失败的可路由代码（`SUBAGENT_AUTH`/`SUBAGENT_QUOTA`/`SUBAGENT_PROVIDER`/`SUBAGENT_PROTOCOL`）通过既有的[结构化错误分类法](../../../.agents/notes/implemented/architecture/2026-06-11-structured-error-taxonomy.md)带到 `ToolExecutionResult.error.info` 上。完整决策，包括仅信任终态结果会踩中的实测 Codex 重试阶梯陷阱，参见[失败分类 Agent Note](../../../.agents/notes/implemented/feature/2026-08-17-subagent-delegation-failure-classification.md)。

`changedFiles`（子 agent 实际写入／创建／删除的绝对路径，已去重）与 `usage`（`{ inputTokens, outputTokens, cacheReadTokens, cacheWriteTokens }`，`inputTokens` 含缓存）同样只由 `codex`／`claude-code` 填充，只出现在 `completed` 或 `max-tokens` 结果上，且提供方未观察到任何变化时会缺省——绝不是空数组。两者都没有能力标志：二者都是结果的**输出**，不是请求的输入，不需要提供方声明支持才能使用，因此 `spawn`／`fork` 根本不会被问及，也就不会填充它们（`failure`／`authMode` 已经确立的"输出无需能力标志"这一推理同样适用于此）。完整证据参见[变更文件与用量 Agent Note](../../../.agents/notes/implemented/feature/2026-08-17-subagent-delegation-changed-files-and-usage.md)所记录的三个实测陷阱——被拒绝的 Claude 工具调用仍会产生 `tool_use` 块；Claude 的 `file_path` 可能是相对路径；Codex 的 `thread/tokenUsage/updated` 每个轮次会多次触发，其 `total` 字段是*线程*生命周期内的累计用量，而不是按通知的增量（[续接 Agent Note](../../../.agents/notes/implemented/feature/2026-08-18-subagent-delegation-resume.md)有完整的实测记录，包括为何 `total` 不会在 `thread/resume` 之后重置）。
...
- **`changedFiles`／`usage` 在 `aborted` 或未分类的 `error` 结果上缺省**：两者只在提供方直接构造 `completed`／`max-tokens` 结果处附加（Codex 为 `wire.runTurn()`，Claude 为 `consumeClaudeQuery()` 的成功分支）；共享的 `settleRunResult()` 辅助函数中通用的取消／失败兜底路径（见 `out-of-process.ts`）从不附加二者，因此被取消或未分类失败的运行不会携带任何部分文件或用量统计。`authMode` 没有这一缺口，因为它由 `settleRunResult()` 自身在每个结算分支上统一附加一次。
- **Resume 是可选的、按提供方限定的，不是通用的续接机制**：只有 `codex`／`claude-code` 声明 `resume`；被续接的运行仍会重新 spawn 一个全新的进程外子 agent，再按 id 重新接入提供方自己持久化的线程／会话，绝不会续接一个 harness 内部的 Activation。`prepareContinuable` 自身的可继续子 agent 机制与之无关，也不受影响——两个特性解决的是不同的问题（持久化、由 harness 拥有的多轮子 agent，对比在 harness 自身回合模型之外由提供方原生续接的线程），并且在同一个工具行上互斥（`dsh-tool-subagent` 会在加载期拒绝 `allowResume` 与 `backgroundMode: 'continuable'` 的组合）。
- **从父级 Session 日志无法重建被续接子级自身的内部状态**：模型可见 ⟺ 已记录这一不变式对父级*自身*所记录的一切依然完全成立（resume id、请求、结果都能忠实地重建），但*子级*自身提供方原生的线程／会话会跨每一次续接独立演化；重放父级日志并不能重现子级自身持久化状态此刻的内容，这与一个纯粹在 harness 内部（不可续接）的子级不同，后者的全部行为都是其已记录请求的确定性函数。
